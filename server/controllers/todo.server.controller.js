...
export const getTodos = (req,res) => {
  Todo.find().exec((err,todos) => {
    if(err){
    return res.json({'success':false,'message':'getTodos Error'});
    }
...
export const addTodo = (req,res) => {
  ...
  newTodo.save((err,todo) => {
    if(err){
    return res.json({'success':false,'message':'addTodo Error'});
    }
...
export const updateTodo = (req,res) => {
  Todo.findOneAndUpdate({ _id:req.body.id }, req.body, { new:true }, (err,todo) => {
    if(err){
    return res.json({'success':false,'message':'updateTodo Error','error':err});
    }
    console.log(todo);
...
export const getTodo = (req,res) => {
  Todo.find({_id:req.params.id}).exec((err,todo) => {
    if(err){
    return res.json({'success':false,'message':'getTodo Error'});
    }
    if(todo.length){
...
export const deleteTodo = (req,res) => {
  Todo.findByIdAndRemove(req.params.id, (err,todo) => {
    if(err){
    return res.json({'success':false,'message':'deleteTodo Error'});
    }
