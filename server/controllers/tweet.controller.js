...
export const getTweet = (req, res) => {
  ...
  Tweets.find({ _id: req.params._id }).exec((err, tweet) => {
    ...
    if (tweet.length) {
      return res.json({
        success: true,
        message: "Tweet fetched by id successfully",
        tweet,
      });
    } else {
      return res.json({
        success: false,
        message: "Tweet with the given id not found",
      });
    }
  });
};
export const deleteTweet = (req, res) => {
  ...
  Tweets.findByIdAndRemove(req.params.id, (err, tweet) => {
    ...
    return res.json({
      success: true,
      message: tweet._id + " deleted successfully",
    });
  });
};
export const getTweetsByUser = (req, res) => {
  ...
  Tweets.find({ "user._id": { $eq: qs.userId } }, (err, tweets) => {
    ...
    return res.json({
      success: true,
      message: "Tweets for " + qs.userId + " fetched successfully",
      tweets,
    });
  });
};

// export const getLikedTweets = (req, res) => {
